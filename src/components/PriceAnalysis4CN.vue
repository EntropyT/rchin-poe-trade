      ...
      <thead class="thead-dark">
        <tr>
          <th scope="col">前 {{ fetchResultPrice.length }} 笔价格分析
            ...
            <br>
            <b-button v-if="searchTotal > 40 && fetchResultPrice.length <= 40" @click="priceAnalysis(8)" :disabled="isCounting" size="sm" variant="outline-light">再多搜 {{ calResultLength >= 40 ? 40 : calResultLength }} 笔价格</b-button>
          </th>
        </tr>
      </thead>
      <tbody @mouseleave="hoveredIndex = -1">
        <tr v-for="(item, index) in collectionCurrency" :key="index">
          <td :style="`opacity: ${hoveredIndex == index ? 1 : parseFloat(item.accountName.length / item.count)};`" @mouseover="handleHover(index)">
            ...
            / <b>{{ item.count }}</b>笔（<b>{{ item.accountName.length }}</b>人标）
    ...
    priceAnalysis(indexLength) {
      ...
      this.axios.all(newFilterResult.map((element, index) => {
        ...
        .catch(function (error) {
          ...
          vm.$message({
            type: 'error',
            message: `被 Server 限制发生请求了，请等待 ${limitState} 秒后再重试`
          });
        })
    },
